...
- **Caixa de e-mails da Loja (16/07/2026):** página `/secretaria/emails` (Secretário e VM) lê a caixa de entrada do Gmail da Loja via **IMAP com a mesma App Password do envio** (`src/lib/gmail-inbox.ts`, imapflow + mailparser — sem depender da publicação do app OAuth): lista as últimas 25 mensagens (não lidas em destaque), abre a mensagem (marca como lida), baixa anexos (`/api/emails/anexo`) e **responde na mesma conversa** (Re: com In-Reply-To/References via SMTP existente). Item de menu "E-mails da Loja" e atalho na página de Pranchas para conferir respostas da Guarda dos Selos.

- **Receitas manuais e categorias financeiras (16/07/2026):** model `CategoriaFinanceira` (tags por Loja e tipo, com 8 categorias padrão semeadas); formulário **"Lançar receita"** no Balancete (tronco, eventos, doações — direto no livro-caixa) e select de categoria no lançamento de despesas, ambos com criação de categoria on-the-fly (`CategoriaSelect`); a categoria da despesa propaga ao livro-caixa no pagamento e a capitação automática lança como "Capitação". Balancete ganhou a **tabela "Consolidado por categoria"** (totais do mês por tag, também no CSV exportado) e card de gestão das categorias.

- **Tooltips de contexto e acessibilidade (16/07/2026):** componente `InfoDica` — ícone (i) ao lado dos títulos das ~17 telas principais e de cards-chave (nível de acesso, cargo do rito, frequência) que abre um popover com explicação em linguagem simples (textos em `src/lib/ajuda.ts`, incluindo glossário: Quitte Placet, Placet, interstício, prancha…); acessível (aria-expanded, fecha com Esc/clique fora, funciona por toque). Acessibilidade global no `globals.css`: foco visível consistente (`:focus-visible`) e, em telas de toque, alvos de clique ≥ 44px e fonte ≥ 16px nos campos (evita zoom automático do iOS).
